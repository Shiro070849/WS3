...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    // สร้างชื่อไฟล์: companyId_type_timestamp.ext
    const companyId = req.query.companyId || 'global';
    const type = req.body.type || 'image'; // logo, favicon
    const timestamp = Date.now();
    const ext = path.extname(file.originalname);
    cb(null, `${companyId}_${type}_${timestamp}${ext}`);
  }
});
