...
async function generateQuotationNumber() {
    try {
        ...
        const currentPrefix = `QTF${year}${month}`; 
        ...
        return `QTF${year}${month}${day}${hour}${minute}`;
    }
}
...
async function convertToNewQuotationFormat(oldNumber) {
    try {
        ...
        if (!oldNumber) {
            console.log("No old number provided, generating new QTF number from database...");
            return await generateQuotationNumber();
        }
        
        // ถ้ามีเลขเก่าแล้วและเป็นรูปแบบ QTF ให้ใช้เลขเก่า
        if (oldNumber.startsWith('QTF') && oldNumber.length >= 11) {
            console.log("Old number is already in QTF format:", oldNumber);
            return oldNumber;
        }
        
        // ถ้าเป็นรูปแบบอื่น (เช่น DO, หรือรูปแบบเก่า) ให้สร้างใหม่
        console.log("Old number format not supported, generating new QTF number:", oldNumber);
        return await generateQuotationNumber();
        ...
        if (oldNumber && oldNumber.startsWith('QTF')) {
            return oldNumber;
        }
...
/**
 * แปลงข้อมูลจาก backend เป็นรูปแบบสำหรับ PDF - เอา VAT ออกและปรับปรุงการจัดการข้อมูลส่วนลด
 */
async function convertBackendDataToPDFFormat(backendData) {
    ...
    // ฟังก์ชันแปลง DD.MM.YYYY → DD.MM.YY
    ...
    const grandTotal = totalAfterProductDiscount - additionalDiscount;
    ...
    return {
        ...
        // ฟิลด์สำหรับ Food Product
        projectInfo: backendData.projectInfo || backendData.project || backendData.Project || "",
        remarkInfo: backendData.remarkInfo || "",
        
        // ฟิลด์สำหรับ Mechanical Product (เก็บไว้เพื่อความเข้ากันได้)
        detailInfo: backendData.detailInfo || "",
        ...
        additionalDetail: backendData.additionalDetail || "",
        
        products: products,
        includeVAT: backendData.includeVAT || false,
        summary: {
            ...
            afterDiscount: grandTotal.toLocaleString('en-US', { 
                minimumFractionDigits: 2, 
                maximumFractionDigits: 2 
            }),
...
<body style="font-family: 'Prompt', 'TH SarabunPSK', Arial, sans-serif; font-size: 9px; line-height: 1.2; color: black; margin: 0; padding: 0; background: white;">`;
   ...
   <!-- หน้า ${totalPages}: Terms & Conditions -->
    <div style="width: 210mm; height: 297mm; margin: 0; background: white; padding: 10mm; box-sizing: border-box; overflow: hidden;">
        ...
        ${createAdditionalDetailsSection(quotationData)}
    ...
    return `
        <thead>
            <tr style="background-color: #f3f4f6;">
                ...
                <th style="border: 1px solid black; padding: ${cellPadding}; text-align: center; font-weight: 600; font-size: ${headerFontSize}; line-height: 1.2; width: 20%;">
                    รหัสสินค้า<br>P/N
                </th>
                <th style="border: 1px solid black; padding: ${cellPadding}; text-align: center; font-weight: 600; font-size: ${headerFontSize}; line-height: 1.2; width: 18%;">
                    ชื่อไทย<br>Thai Name
                </th>
                <th style="border: 1px solid black; padding: ${cellPadding}; text-align: center; font-weight: 600; font-size: ${headerFontSize}; line-height: 1.2; width: 15%;">
                    Brand<br>แบรนด์
                </th>
...
function createProductRows(products, startIndex = 1, fontSize = '9px', cellPadding = '5px', rowHeight = '25px') {
    return products.map((product, index) => {
        ...
        const brand = product.brand || product.I_ItemDescriptionEN || '';
        const thaiName = product.thaiName || product.Description || '';  
        ...
        <tr style="height: ${rowHeight};">
            ...
            <td style="border: 1px solid black; text-align: left; padding-left: ${cellPadding}; vertical-align: middle; font-size: ${isCompact ? '7px' : fontSize}; word-wrap: break-word; overflow-wrap: break-word;">
                ${thaiName}
            </td>
            <td style="border: 1px solid black; text-align: left; padding-left: ${cellPadding}; vertical-align: middle; font-size: ${isCompact ? '7px' : fontSize}; word-wrap: break-word; overflow-wrap: break-word;">
                ${brand}
            </td>
            <td style="border: 1px solid black; text-align: center; vertical-align: middle; font-size: ${fontSize}; padding: ${cellPadding}; word-wrap: break-word;">
                ${size}
            </td>
            <td style="border: 1px solid black; text-align: center; vertical-align: middle; font-size: ${fontSize}; padding: ${cellPadding};">
                ${product.quantity || '0'}
...
function createSummaryRows(totalQuantity, summary, fontSize = '9px', cellPadding = '5px', rowHeight = '28px', quotationData = {}) {
    // ตรวจสอบว่าต้องแสดง VAT หรือไม่
    const showVAT = quotationData.includeVAT === true;
    
    // คำนวณยอดเงิน
    let subtotalValue = parseFloat(summary.subtotal.replace(/,/g, '')) || 0;
    let vatAmount = 0;
    let grandTotalValue = subtotalValue;
    
    if (showVAT) {
        vatAmount = subtotalValue * 0.07; // คิด VAT 7%
        grandTotalValue = subtotalValue + vatAmount;
    }
    
    // Format ตัวเลข
    const formattedSubtotal = subtotalValue.toLocaleString('en-US', { 
        minimumFractionDigits: 2, 
        maximumFractionDigits: 2 
    });
    
    const formattedVAT = vatAmount.toLocaleString('en-US', { 
        minimumFractionDigits: 2, 
        maximumFractionDigits: 2 
    });
    
    const formattedGrandTotal = grandTotalValue.toLocaleString('en-US', { 
        minimumFractionDigits: 2, 
        maximumFractionDigits: 2 
    });
    ...
    let html = `
        <tr style="height: ${rowHeight};">
            <td style="border: 1px solid black; text-align: center; padding: ${cellPadding}; font-weight: 500; background-color: #e5e7eb; font-size: ${fontSize};" colspan="3">
                หมายเหตุ / Remarks
            </td>
            ...
            <td style="border: 1px solid black; text-align: center; padding: ${cellPadding}; vertical-align: middle; font-size: ${fontSize}; background-color: white;" colspan="1">
                รวมเป็นเงิน / Total
            </td>
        ...
        <tr style="height: ${rowHeight};">
            ...
            <td style="border: 1px solid black; padding: ${cellPadding}; background-color: white; font-size: ${fontSize};" colspan="7">
                ${quotationData.projectInfo || ''}
            </td>
        </tr>
        <tr style="height: ${rowHeight};">
            <td style="border: 1px solid black; padding: ${cellPadding}; font-weight: 500; font-size: ${fontSize}; background-color: white;">Remark</td>
            <td style="border: 1px solid black; padding: ${cellPadding}; background-color: white; font-size: ${fontSize};" colspan="7">
                ${quotationData.remarkInfo || ''}
            </td>
        </tr>`;
    
    // เพิ่มแถว VAT ถ้าเปิดโหมด VAT
    if (showVAT) {
        html += `
        <tr style="height: ${rowHeight};">
            <td style="border: 1px solid black; text-align: center; padding: ${cellPadding}; vertical-align: middle; font-size: ${fontSize}; background-color: white;" colspan="7">
                VAT 7%
            </td>
            <td style="border: 1px solid black; text-align: right; padding-right: ${cellPadding}; vertical-align: middle; font-size: ${fontSize}; background-color: white;">
                ${formattedVAT}
    ...
    html += `
        <tr style="height: ${rowHeight};">
            <td style="border: 1px solid black; text-align: center; padding: ${cellPadding}; vertical-align: middle; font-weight: 600; font-size: ${fontSize}; background-color: white;" colspan="7">
 ...
 */
function createAdditionalDetailsSection(quotationData) {
    const displayContent = quotationData.additionalDetail || '-';
    
    return `
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 8px; font-size: 8px;">
            <tr>
                <td style="border: 1px solid black; background-color: #f0f0f0; padding: 6px; font-size: 10px; font-weight: 600;">
                    รายละเอียดเพิ่มเติม / Additional Details
                </td>
            </tr>
            <tr>
                <td style="border: 1px solid black; padding: 6px; text-align: left; line-height: 1.5;">
                    ${displayContent}
                </td>
            </tr>
        </table>
...
function createPaymentTermsSection(grandTotal, grandTotalNumber = 0, quotationData = {}) {
    // คำนวณยอดเงินใหม่ถ้ามี VAT
    const showVAT = quotationData.includeVAT === true;
    let finalTotal = grandTotalNumber;
    
    if (showVAT && grandTotalNumber > 0) {
        finalTotal = grandTotalNumber * 1.07; // เพิ่ม VAT 7%
    }
                ...
                <tbody>
                    ...
                    <tr style="font-weight: 700; height: 28px; background-color: #f8f9fa;">
                        ...
                        <td style="border: 1px solid black; text-align: center; vertical-align: middle; padding: 4px; font-size: 8px; width: 45%;">
                            ${finalTotal > 0 ? '( ' + numberToThaiText(finalTotal) + ' )' : ''}
                        </td>
 ...
 */
function createEmailTemplate(quotationNo = "QTF250001", customerName = "ลูกค้า") {
    return `
...
async function SendOrder(backendData = null, emailParams = null, uploadedFiles = null) {
    try {
        ...
        const mailOptions = {
            ...
            from: `"Quotation Food" <${emailSettings.username}>`,
            ...
            subject: `รายการเปิดใบเสนอราคา ${quotationData.quotationNo} - ${quotationData.customerName} - FOOD`,
